...
function dwLogWrite($data) {
	file_put_contents("/tmp/microsoftgraph.log",@file_get_contents("/tmp/microsoftgraph.log").$data."\r\n");
}

# DEFINE THE OAUTH PROVIDER AND SETTINGS TO USE #
$_SESSION['WPOA']['PROVIDER'] = 'MicroSoft Graph';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_microsoftgraph_api_enabled'));
define('CLIENT_ID', get_option('wpoa_microsoftgraph_api_id'));
define('CLIENT_SECRET', get_option('wpoa_microsoftgraph_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'user.read mail.read'); // PROVIDER SPECIFIC: 'wl.basic' is the minimum scope required to get the user's id from Windows Live
define('URL_AUTH', "https://login.live.com/oauth20_authorize.srf?");
define('URL_TOKEN', "https://login.live.com/oauth20_token.srf?");

//define('URL_AUTH', "https://login.microsoftonline.com/common/oauth2/v2.0/authorize?");
//define('URL_TOKEN', "https://login.microsoftonline.com/common/oauth2/v2.0/token?");
define('URL_USER', "https://graph.microsoft.com/v1.0/me?");
...
function get_oauth_identity($wpoa) {
	...
	switch (strtolower(HTTP_UTIL)) {
		case 'curl':
			...
			curl_setopt($curl, CURLOPT_HTTPHEADER, array(
				...
				,'x-li-format: json'
			); // PROVIDER SPECIFIC: do we have to do this for MicroSoft Graph?
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			$result = curl_exec($curl);
			file_put_contents("/tmp/microsoftgraph.log",$result);
	...
	$oauth_identity['email'] = $result_obj['userPrincipalName']; // PROVIDER SPECIFIC: Windows Live never provides the user's email!
